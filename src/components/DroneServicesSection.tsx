...
export const DroneServicesSection: React.FC<DroneServicesSectionProps> = ({ onOpenBookingModal }) => {
  ...
  return (
    <section id="services" className="py-12 lg:py-16 bg-white border-t border-slate-200/60" dir={isUrdu ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Prominent Premium Banner for Drone Spray Booking */}
        <div
        ...
        >
          ...
          <div className="relative z-10 space-y-2 text-center md:text-left">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/20 text-purple-200 text-xs font-bold border border-purple-400/30">
              ...
              <span>{isUrdu ? 'جدید زرعی ٹیکنالوجی' : 'Precision Agriculture'}</span>
            </div>
            <h3 className="text-xl sm:text-2xl md:text-3xl font-black text-white tracking-tight">
              {isUrdu ? '🚁 اپنی فصل کا اسپرے اب جدید ڈرون سے کروائیں' : '🚁 Agricultural Drone Spray Service'}
            </h3>
            <p className="text-sm sm:text-base text-emerald-300 font-bold">
              {isUrdu
                ? 'وقت کی بچت، یکساں اسپرے اور فصل کے کچلے جانے کا صفر خطرہ'
                : 'Save time, ensure uniform coverage, and eliminate crop trampling'}
            </p>
          </div>
          ...
          <div className="relative z-10 shrink-0 w-full md:w-auto">
            ...
            >
              ...
              <span>{isUrdu ? '🚁 ڈرون اسپرے بک کریں' : '🚁 Book Drone Spray'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-6">
            <div className="relative rounded-3xl overflow-hidden shadow-lg border border-slate-200 aspect-4/3 group">
              ...
              <div className="absolute bottom-4 left-4 right-4 bg-slate-900/90 backdrop-blur-md rounded-2xl p-4 text-white border border-white/10 flex items-center justify-between shadow-lg transition-transform duration-300 group-hover:translate-y-[-2px]">
                <div>
                  <span className="text-xs text-emerald-400 font-bold">{isUrdu ? 'تیز رفتار سپرے' : 'Fast Coverage'}</span>
                  <div className="text-base sm:text-lg font-extrabold">{isUrdu ? '15-20 منٹ / ایکڑ' : '15-20 Mins / Acre'}</div>
                </div>
                <div className="h-8 w-px bg-white/20" />
                <div>
                  <span className="text-xs text-emerald-400 font-bold">{isUrdu ? 'فصل کا صفر نقصان' : 'Zero Crop Loss'}</span>
                  <div className="text-base sm:text-lg font-extrabold">{isUrdu ? '0% فیلڈ نقصان' : '0% Field Trampling'}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-6">
            <div className="inline-flex items-center gap-2 bg-purple-50 text-purple-700 px-3 py-1 rounded-full text-xs font-bold mb-4 border border-purple-200/60">
              ...
              <span>{isUrdu ? 'جدید ترین زرعی میکانائزیشن' : 'Next-Gen Farm Mechanization'}</span>
            </div>

            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-800 mb-2 tracking-tight">
              {isUrdu ? 'جدید زرعی ڈرون اسپرے سروس' : 'Modern Agricultural Drone Spray Service'}
            </h2>

            <p className="text-sm sm:text-base text-slate-500 mb-4 leading-relaxed">
              {isUrdu
                ? 'کوٹ ادو اور ملحقہ اضلاع کے کسانوں کے لیے جدید ڈرون اسپرے، کم وقت میں وسیع رقبے کی یکساں کوریج اور ادویات کی بچت۔'
            ...
            <p className="text-sm sm:text-base text-slate-700 bg-slate-50 p-5 rounded-2xl border border-slate-200 mb-6 leading-relaxed">
              {isUrdu
                ...
                : 'Modern, fast, and effective drone spraying. Free yourself from the labor of manual knapsack spraying. Downdraft ensures full penetration from top leaves to the roots.'}
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-bold text-slate-800 text-sm">{isUrdu ? 'تیز رفتار تکمیل' : 'Rapid Turnaround'}</div>
                  <div className="text-xs text-slate-500">
                    {isUrdu ? 'گھنٹوں میں درجنوں ایکڑ کا اسپرے بغیر کسی تاخیر کے۔' : 'Spray dozens of acres in hours instead of multiple days.'}
                  </div>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-bold text-slate-800 text-sm">{isUrdu ? 'پانی اور ایندھن کی بچت' : 'Water & Fuel Savings'}</div>
                  <div className="text-xs text-slate-500">
                    {isUrdu ? 'جدید الٹرا لو والیوم نوزلز پانی کے استعمال کو نمایاں کم کرتی ہیں۔' : 'Ultra-low volume nozzles drastically cut water requirements.'}
                  </div>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-bold text-slate-800 text-sm">{isUrdu ? 'مکمل اور یکساں رسائی' : 'Precision Penetration'}</div>
                  <div className="text-xs text-slate-500">
                    {isUrdu ? 'پروپیلر کا دباؤ دوا کے باریک قطرے گھنے پتوں کے اندر تک پہنچاتا ہے۔' : 'Propeller downdraft forces mist deep into dense foliage.'}
                  </div>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-bold text-slate-800 text-sm">{isUrdu ? 'محفوظ طریقہ کار' : 'Safe for Labor'}</div>
                  <div className="text-xs text-slate-500">
                    {isUrdu ? 'کسانوں اور مزدوروں کو زہریلی ادویات کے مضر اثرات سے مکمل تحفظ۔' : 'Eliminates toxic chemical exposure to field laborers.'}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              ...
              <a
                ...
                href={buildWhatsAppUrl(
                  isUrdu
                    ? 'السلام علیکم! کسان ایگرو ٹریڈرز، مجھے زرعی ڈرون اسپرے سروس کے ریٹس اور شیڈول کی معلومات درکار ہے۔'
                    : 'Hello Kissan Agro Traders! What is the rate per acre and schedule for Drone Spray service?'
                )}
                target="_blank"
                rel="noopener noreferrer"
              >
                <span className="material-symbols-outlined text-[19px]">chat</span>
                <span>{isUrdu ? 'واٹس ایپ پر ریٹ پوچھیں' : 'WhatsApp Rates'}</span>
              </a>
              ...
              >
                ...
                <span>{isUrdu ? 'ہیلپ لائن پر کال کریں' : 'Call Helpline'}</span>
              </a>
            </div>
            ...
            <div className="mt-4 inline-flex items-center gap-2 py-1.5 px-3.5 rounded-xl bg-emerald-50 border border-emerald-200/70 text-emerald-800 text-xs font-bold">
              ...
              <span>{isUrdu ? '🚚 مفت فارم معائنہ و ڈیلیوری' : '🚚 Free Farm Inspection & Delivery'}</span>
            </div>
          </div>
        </div>
      </div>
